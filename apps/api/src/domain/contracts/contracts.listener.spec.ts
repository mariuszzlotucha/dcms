...
describe('ContractsListener', () => {
  ...
  it('moves the contract to approved on approval.granted', async () => {
    ...
    await listener.handleApprovalGranted({
      contractId: 'c1',
      tenantId: 't1',
  ...
  it('swallows an invalid-transition error (e.g. a second approver granting after the first already moved it)', async () => {
    contractsService.changeStatus.mockRejectedValue(
      new BadRequestException('Contract is already "approved"'),
    );

    await expect(
      listener.handleApprovalGranted({
        contractId: 'c1',
        tenantId: 't1',
