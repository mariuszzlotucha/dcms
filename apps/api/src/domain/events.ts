...
export interface DomainEventPayloadMap {
  [DOMAIN_EVENTS.CONTRACT_CREATED]: {
    contractId: string;
    tenantId: string;
  ...
  [DOMAIN_EVENTS.CONTRACT_VERSION_CREATED]: {
    contractId: string;
    tenantId: string;
    versionId: string;
    createdBy: string;
  };
  [DOMAIN_EVENTS.CONTRACT_STATUS_CHANGED]: {
    contractId: string;
    tenantId: string;
    previousStatus: string;
    newStatus: string;
  };
  [DOMAIN_EVENTS.CONTRACT_SUBMITTED_FOR_APPROVAL]: {
    contractId: string;
    tenantId: string;
    submittedBy: string;
  };
  ...
  [DOMAIN_EVENTS.TEMPLATE_CLAUSE_LIBRARY_UPDATED]: {
    templateId: string;
    tenantId: string;
    clauseId: string;
  };
  ...
  [DOMAIN_EVENTS.NEGOTIATION_REVISION_REQUESTED]: {
    contractId: string;
    tenantId: string;
    requestedBy: string;
    comment: string;
  };
  [DOMAIN_EVENTS.NEGOTIATION_ROLE_ASSIGNED]: {
    contractId: string;
    tenantId: string;
    userId: string;
  ...
  [DOMAIN_EVENTS.ESIGNATURE_REQUESTED]: {
    contractId: string;
    tenantId: string;
    envelopeId: string;
    provider: 'docusign' | 'adobe_sign';
  };
  [DOMAIN_EVENTS.ESIGNATURE_SENT]: {
    contractId: string;
    tenantId: string;
    envelopeId: string;
    recipientEmail: string;
  };
  [DOMAIN_EVENTS.ESIGNATURE_COMPLETED]: {
    contractId: string;
    tenantId: string;
    envelopeId: string;
    completedAt: Date;
  };
  [DOMAIN_EVENTS.ESIGNATURE_DECLINED]: {
    contractId: string;
    tenantId: string;
    envelopeId: string;
    reason: string;
  ...
  [DOMAIN_EVENTS.COLLABORATION_PARTICIPANT_INVITED]: {
    contractId: string;
    tenantId: string;
    invitedEmail: string;
    invitedBy: string;
  };
  [DOMAIN_EVENTS.COLLABORATION_COMMENT_ADDED]: {
    contractId: string;
    tenantId: string;
    commentId: string;
    authorId: string;
  };
  [DOMAIN_EVENTS.COLLABORATION_SESSION_STARTED]: {
    contractId: string;
    tenantId: string;
    userId: string;
  };

  [DOMAIN_EVENTS.COMPLIANCE_REPORT_GENERATED]: {
    reportId: string;
    tenantId: string;
    contractId: string | null;
    generatedBy: string;
  };
  [DOMAIN_EVENTS.COMPLIANCE_REPORT_EXPORTED]: {
    reportId: string;
    tenantId: string;
    format: 'pdf' | 'csv';
  };
  ...
  [DOMAIN_EVENTS.REMINDER_SCHEDULED]: {
    tenantId: string;
    contractId: string;
    reminderType: string;
    scheduledFor: Date;
  ...
  [DOMAIN_EVENTS.NOTIFICATION_DISPATCHED]: {
    tenantId: string;
    recipient: string;
  ...
  [DOMAIN_EVENTS.INTEGRATION_SYNC_REQUESTED]: {
    tenantId: string;
    integration: 'salesforce' | 'hubspot' | 'google-drive' | 'ms365';
    contractId: string | null;
  };
  [DOMAIN_EVENTS.INTEGRATION_SYNC_COMPLETED]: {
    tenantId: string;
    integration: string;
    contractId: string | null;
  };
